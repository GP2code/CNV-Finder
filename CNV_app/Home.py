...
def plot_sample():
    sample_df_interval = pd.read_csv(
        f"CNV_app/data/{st.session_state['model_choice']}/{st.session_state['cohort_choice']}/{st.session_state['gene_choice']}/app/pred_cnvs/{st.session_state['sample_name']}_full_interval.csv")
...
model_name = st.sidebar.selectbox(
    label='Model Selection', label_visibility='collapsed', options=models)
...
if not os.path.isfile(model_path):
    ...
    with st.expander("Filter Displayed Samples"):
        # update later to increment from minimum predicted value available
        probab_options = [0.5, 0.55, 0.6, 0.65,
                          0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1]
        confidence = st.select_slider(
            'Display samples with prediction probability of at least:', options=probab_options, value=1)

        # make all threshold selections cleaner - format decimals
        iqr_range = np.linspace(min(abs(model_results['abs_iqr_lrr'])), max(
            abs(model_results['abs_iqr_lrr'])), num=50)
        iqr_threshold = st.select_slider(
            'Maximum Absolute Value LRR range threshold:', options=iqr_range, value=max(iqr_range))
        ...
        lower_range_threshold = st.select_slider(
            'Minimum threshold for CNV count:', options=lower_range, value=selected_value_lower)
        upper_range_threshold = st.select_slider(
            'Maximum threshold for CNV count :', options=upper_range, value=selected_value_higher)

        exp1, exp2, exp3 = st.columns([1.5, 1, 1])
        st.session_state['threshold_submit'] = exp2.checkbox(
    ...
    if st.session_state['threshold_submit']:
        ...
        # display max value possible if > 0.95 is unavailable
        max_pred = max(model_results['Pred Values'])
        if max_pred < 0.95:
            cohort_samples = model_results.IID[model_results['Pred Values'] == max_pred]
            st.info(
                f'No samples with predicted values over 0.95, please adjust prediction probability threshold to be less than {format(max_pred, ".2f")}. Currently displaying samples with predicted values of at least {format(max_pred, ".2f")}.')
        else:
            cohort_samples = model_results.IID[model_results['Pred Values'] >= 0.95]
    ...
    if yes:
        ...
        st.session_state['yes_type'].append(
            st.session_state["model_choice"].split('_')[0])
    elif maybe:
        samples_seen = f'{st.session_state["gene_choice"]}_{st.session_state["cohort_choice"]}_{st.session_state["model_choice"]}sample_seen'
        st.session_state[samples_seen].append(st.session_state['sample_name'])
