...
def main():
    ...
    if create_test:
        ...
        with multiprocessing.Pool(cpus) as pool:
            pool.map(fill_window_df, [(out_path, row.IID, row.snp_metrics_path, split_interval, total_windows, cnv_exists, chrom, start_pos, stop_pos, buffer, min_gentrain, bim, pvar) for index, row in test_df.iterrows()])
    ...
    if create_train:
        ...
        for i in range(len(train_df.iid)):
            ...
            if "interval" in train_df.columns:
                chrom, start, stop = check_interval(train_df.interval[train_df.iid == sample].values[0])
                train_df.chr.iloc[i] = chrom
                train_df.start.iloc[i] = start
                train_df.stop.iloc[i] = stop
            elif interval_name:
                train_df[['chr', 'start', 'stop']] = check_interval(interval_name)
            else:
        ...
        # Parallelize creation of df that holds all samples with aggregated feature in each window
        with multiprocessing.Pool(cpus) as pool:
            pool.map(fill_window_df, [(out_path, row.iid, row.snp_metrics_path, split_interval, total_windows, row.cnv_exists, row.chr, row.start, row.stop, buffer, min_gentrain, bim, pvar) for index, row in train_df.iterrows()])
